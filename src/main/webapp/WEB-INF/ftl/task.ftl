...
<script type="text/javascript">
	...
	function detailTask(){
		...
		if (row) {
			...
			string += "<tr><td>任务描述</td>" + "<td><textarea disabled='disabled' rows='5' cols='20' style='resize:none'>" + row.task_desc+"</textarea></td></tr>";
			string += "<tr><td>完成情况</td>" + "<td><textarea disabled='disabled' rows='5' cols='20' style='resize:none'>" + row.finish_desc+"</textarea></td></tr>";
			string += "</table>"
	...
	function saveuser() {
		...
		if($("#fm input[name='task_name']").val().length < 1 || $("#fm textarea[name='task_desc']").val().length > 128 || $("#fm textarea[name='finish_desc']").val().length > 128){
			$.messager.alert("提示信息", "请正确输入");
			return;
		}
		$.ajax({
			type: "POST",
...
<body>
	<div style="float:right;">${username!'匿名用户'},你好。点击右边按钮退出<input type="button" value="注销" onclick="javascript:location.href='logout.html'"/></div>
	<form id="queryTime">
	...
	<input type="button" onclick="queryInfo();"  style="width:460px" value="查询" />
	<input id="queryUser" type="hidden" value="" />
	<hr  color=white width=460 align=left />
